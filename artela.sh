...
# 节点安装功能
    ...
    if ! check_go_installation; then
        ...
        echo 'export PATH=$PATH:/usr/local/go/bin:$HOME/go/bin' >> /mnt/c1/.bash_profile
    ...
    # 获取初始文件和地址簿
    curl -L https://snapshots-testnet.nodejumper.io/artela-testnet/genesis.json > /mnt/c1/.artelad/config/genesis.json
    curl -L https://snapshots-testnet.nodejumper.io/artela-testnet/addrbook.json > /mnt/c1/.artelad/config/addrbook.json
    ...
    sed -i 's|^persistent_peers *=.*|persistent_peers = "'$PEERS'"|' /mnt/c1/.artelad/config/config.toml
    ...
    echo "export Artela_RPC_PORT=$node_address" >> /mnt/c1/.bash_profile
    source /mnt/c1/.bash_profile   

    pm2 start artelad -- start && pm2 save && pm2 startup
    
    # 下载快照
    artelad tendermint unsafe-reset-all --home /mnt/c1/.artelad --keep-addr-book
    curl https://snapshots-testnet.nodejumper.io/artela-testnet/artela-testnet_latest.tar.lz4 | lz4 -dc - | tar -xf - -C /mnt/c1/.artelad
    mv /mnt/c1/.artelad/priv_validator_state.json.backup /mnt/c1/.artelad/data/priv_validator_state.json
